...
class TabletopSceneCustom(InitialSceneTemplates):
    ...
    def define_regions(self):
        ...
        self.regions.update(
            self.get_region_dict(
                region_centroid_xy=[-0.15, -0.05-0.10],
...
def main():
    ...
    language = "Place both mugs on a plate that is on the table"
    register_task_info(
        language,
        scene_name=scene_name,
        objects_of_interest=["red_coffee_mug_1", "porcelain_mug_1", "plate_1"],
        goal_states=[
            ("FlexibleOn", "red_coffee_mug_1", "plate_1", 0.07, 0.07),
            ("FlexibleOn", "porcelain_mug_1", "plate_1", 0.07, 0.07),
        ],
    )
